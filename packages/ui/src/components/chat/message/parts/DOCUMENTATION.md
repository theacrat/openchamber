...
- Managed `openchamber` session create and fork calls use the subagent card and read the child identity from the successful result envelope. Other actions retain the generic expandable output. Child navigation resolves the child's directory rather than inheriting its parent's directory.
